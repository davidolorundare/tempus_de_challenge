...
- The Airflow `scheduler` needs to be running alongside the Airflow `webserver`, for the scheduled dag-pipeline to run at their predefined times. This was *very* tricky to setup correctly in the docker container; as the *current* command, in the webserver section, of the docker-compose file starts only the webserver. 
	...
	* Various approaches were experimented with to achieved this desired intent, however they all continued to break the contaier during build-time. The dockerfile for airflow itself has an option to run `scheduler` and `webserver` together - but only if Airflow was running in `LocalExecutor` mode. However, even with that option set in the aiflow.cfg config file, no visible changes were noticed in the container airflow startup process. The remaining options include exploring either placing a `docker run` or injecting some other command-script into the container or exploring use of a service supervisor and manager such as [upstart](https://airflow.readthedocs.io/en/stable/howto/run-with-upstart.html) or [systemd](https://airflow.readthedocs.io/en/stable/howto/run-with-systemd.html) within the docker-container.
