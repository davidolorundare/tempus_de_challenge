...
- The Airflow scheduler needs to be running alongside the Airflow webserver, for the scheduled dag-pipeline to run at their predefined times. This was *very* tricky to setup correctly in the docker container; as the *current* command, in the webserver section, of the docker-compose file starts only the webserver. 
	* Attempts were made to add the extra command to start the scheduler via replacing 'webserver' with a bash command to run 'scheduler and webserver', however this broke the container build process. One of the issues being the scheduler and webserver can't run one after the other in the same docker shell (using the typical bash '&& or &' commands), rather to run concurrently the commands need to be run in separate shells within the same container. 
	* Various approaches were experimented with to achieved this desired intent, however they all continued to break the docker during build-time. The dockerfile for airflow itself had an option to run scheduler and webserver together - but only if Airflow was running in `LocalExecutor` mode. Even with that option set in the aiflow.cfg config file no visible changes were noticed. The remaining options to explore are either placing a `docker run` some in the docker script or to explore configuring within the docker-container a service supervisor and manager such as [upstart](https://airflow.readthedocs.io/en/stable/howto/run-with-upstart.html) or [systemd](https://airflow.readthedocs.io/en/stable/howto/run-with-systemd.html).
